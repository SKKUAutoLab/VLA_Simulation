...
OT_DIST = float(os.environ.get("OT_DIST", "7.0"))           # 추월: 옆차선 이동 후 이만큼 전진하면 원차선 복귀(m)
...
class VLALoraNode(Node):
    def __init__(self):
        ...
        self.overtaking = False; self.ot_origin = None; self.ot_from = None  # 추월: 원차선 + 시작위치
    ...
    def _cmd(self, msg):
        ...
        # 추월 — 옆차선으로 나가 OT_DIST 전진(앞차 지나침) 후 원차선 복귀. (회피=눌러앉음, 추월=복귀)
        if any(k in t for k in ("추월", "앞지르", "앞질러", "overtake")):
            if self.cur_p is None:
                self.get_logger().warn("[lora] 추월 실패: 위치 미수신"); return
            other = 1 - self.keep_lane
            with self.lock:
                self.overtaking = True; self.ot_origin = self.keep_lane; self.ot_from = self.cur_p
                self.keep_lane = other; self.lane = other; self.approach_lane = other
                self.active = True; self.paused = False
                self.start_idx = None; self.visited = set(); self._last = None
            self.get_logger().info(f"[lora] ↪ 추월 시작 → {other+1}차선 이동, {OT_DIST}m 전진 후 복귀"); return
        ...
        if any(k in t for k in ("멈춰", "정지", "stop")):
            with self.lock:
                ...
                self.reversing = False; self.overtaking = False   # 후진·추월 중이면 취소
    ...
    def _maybe_avoid(self):
        ...
        if not (self.active and not self.paused and self.direct_target is None and self.lane in (0, 1)) \
                or self.overtaking:   # 추월 진행 중엔 자동회피 억제(수동 차선제어와 충돌 방지)
            return
    ...
    def _pub(self):
        ...
        # 추월 진행: 옆차선에서 OT_DIST 만큼 전진(앞차 지나침)하면 원차선으로 복귀. (제어는 아래 정상경로 유지)
        if self.overtaking:
            with self.lock:
                passed = (self.cur_p is not None and self.ot_from is not None
                          and math.dist(self.cur_p, self.ot_from) >= OT_DIST)
                if passed:
                    self.overtaking = False
                    self.keep_lane = self.ot_origin; self.lane = self.ot_origin
                    self.approach_lane = self.ot_origin
                    self.start_idx = None; self.visited = set(); self._last = None
                    origin = self.ot_origin
            if passed:
                self.get_logger().info(f"[lora] ↩ 추월 완료 → {origin+1}차선 복귀")
        with self.lock:
