...
```bash
cd ~/VLA_simulation/lora_pipeline    # 이하 모든 python3 명령은 워크스페이스 루트에서 실행
```

> ⚠️ **경로 주의:** `lora_pipeline`은 `~/lora_pipeline`이 아니라
> **`~/VLA_simulation/lora_pipeline`** 에 있습니다.
> 아래 `python3 lora_pipeline/...` 명령은 모두 **워크스페이스 루트(`~/VLA_simulation`)**
...
```bash
source ~/VLA_simulation/install/local_setup.bash
ros2 topic pub --once /vla/command std_msgs/String "{data: '1차선 한바퀴 돌아'}"
# 정지:            "{data: '멈춰'}"
# 2차선 주행:      "{data: '2차선 한바퀴 돌아'}"
# 차선 변경:       "{data: '2차선으로 바꿔'}"
# 목적지 정지:      "{data: '횡단보도에서 정지'}"   (출발점/주차구역도 가능)
```

**행동 공간 (지원 명령 모음)** — 자동차가 할 수 있는 동작. `vla_brain_node`가 자유 문장을 아래 표준 명령으로 변환합니다.

| 동작 | 예시 명령 | 설명 |
|------|-----------|------|
| 주행 | `1차선 계속 돌아`, `2차선 세 바퀴 돌아` | 차선·랩수 지정 비전 추종 |
| 차선 변경 | `2차선으로 바꿔` | 다른 차선으로 전이 |
| 정지 | `멈춰` | 즉시 정지 |
| 일시정지/재개 | `잠깐`, `재개` | 랩 상태 유지하며 멈춤/이어감 |
| 목적지 정지 | `횡단보도에서 정지` | 랜드마크 도달 시 정지 |
| **후진** | `뒤로`, `후진` | 음수 속도로 2m 물러난 뒤 자동 정지 |
| **속도 조절** | `천천히`/`서행`, `빨리`/`빠르게`, `보통 속도` | 순항 속도 배율(0.6x / 1.3x / 1.0x) |
| **추월** | `추월`, `앞질러` | 옆 차선으로 나가 앞차를 지나친 뒤 원 차선 복귀 |
| 방향 전환 | `역방향으로 돌아` | 트랙 진행 방향 전환(텔레포트 스냅) |
| 복구 | `복구`, `리셋` | 가장 가까운 차선으로 스냅 |

> 참고: 앞에 장애물이 있으면 라이다가 **자동으로 옆 차선 회피**(눌러앉음)합니다. **추월**은 이와 달리 지나친 뒤 **원 차선으로 복귀**하는 능동 기동입니다.
> `vla/command`는 키워드로 해석됩니다. 완전 자유형 문장은 `vla_brain_node`(Qwen3-VL)가 `/nl_command`로 받아 표준 명령으로 변환합니다.
...
1. **경로 혼동:** `lora_pipeline`은 `~/lora_pipeline`이 아니라
   `~/VLA_simulation/lora_pipeline`. `python3 lora_pipeline/...`는 워크스페이스 루트에서.
...
**워크플로우 (마스크 → 중앙선 추출)**
1. **1차선/2차선** 선택 후, 트랙 위 차선 영역의 **테두리를 점으로 클릭** → 점들이 이어져 닫힌 **면(폴리곤)이 반투명 색으로 채워짐** (1차선 빨강 / 2차선 파랑). 시작점(초록) 근처에서 자동 스냅으로 깔끔하게 닫힘.
2. **중앙선 추출**(버튼 또는 `e`) → 칠한 면의 **바깥·안쪽 경계 사이 정확한 가운데선**을 자동 계산(흰 선).
3. **Save**(`s`) → 파일 저장창에서 이름 선택. 기본값은 주행 노드가 읽는 경로(`track_gt_lane1_demo.json`=1차선 / `track_gt_lane0_demo.json`=2차선). 라벨마스크(`<이름>_label.png`, 0 배경/1/2)도 함께 저장.

**조작:** 점 추가(빈 곳 딸깍) · 선 위 삽입(선에 딸깍) · 이동(점 드래그) · 삭제(점 우클릭) · 화면이동(빈 곳 드래그) · 확대(휠) · 되돌리기(`u`/Ctrl+Z) · 불러오기(`l`, JSON/라벨PNG).
